...
class UserFavoriteDrinkService {
  ...
  //final userFavoriteDrinkCollection = FirebaseFirestore.instance.collection('Users').doc(User.uid).collection('user_favorite');

  Future<void> checkExits() async{
    bool exists = false;
    await FirebaseFirestore.instance.collection('Users').doc(uid).collection('user_favorite').get().then((value){value.docs.forEach((element) { if(element.id == 'favorite_drink') exists = true;});});
    if(!exists){
      List<dynamic> lists = [];
      await FirebaseFirestore.instance.collection('Users').doc(uid).collection('user_favorite').doc('favorite_drink').set({'drink_list': lists});
    }
  }
  ...
  Future<bool> checkFavoriteDrinkExists(UserFavoriteDrink userFavoriteDrink) async{
    var wait = await FirebaseFirestore.instance.collection('Users').doc(uid).collection('user_favorite').doc('favorite_drink').get();
    bool ret = false;
    if(wait.exists){
  ...
  Future<void> addNewUserFavoriteDrink(UserFavoriteDrink userFavoriteDrink) async{
    ...
    if(!wait.exists){
      ...
      await FirebaseFirestore.instance.collection('Users').doc(uid).collection('user_favorite').doc('favorite_drink').update({'drink_list': lists});
    }
  }
  //READ
