...
class UserFormFirstPage extends StatefulWidget {
  const UserFormFirstPage({Key? key,}) : super(key: key);

  @override
  State<UserFormFirstPage> createState() => _UserFormState();
}

class _UserFormState extends State<UserFormFirstPage> {
  ...
  void initState() {
    ...
    _nameController.addListener(() {
      userProvider.userName = _nameController.text;
    });

    _ageController.addListener(() {
      userProvider.userAge = int.tryParse(_ageController.text) ?? 0;
    });
  }
                                ...
                                Expanded(
                                  child: TextFormField(
                                    ...
                                    onChanged: (value){
                                      ...
                                      setState(() {
                                        userProvider.userAge = int.parse(value);
                                      });
                                    },
                                  ),
                                ),
                ...
                child: ElevatedButton(
                    onPressed: (){
                      if(_formKey.currentState?.validate() ?? false) {
                        Navigator.push(
                          context,
                          MaterialPageRoute(builder: (context) => UserFormSecondPage(userProvider : userProvider)),
                        );
                      }
                    },
                    child: Text('다음')),
